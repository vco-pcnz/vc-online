<template>
  <div>
    <!-- 人员选择 -->
    <vco-choose-user ref="vcoChooseUserRef" :isMultiple="true" :title="t('利益相关者')" url="stake/selStake" @done="userChoiced">
      <div></div>
    </vco-choose-user>

    <a-modal :open="visible" :title="title" :width="700" :footer="null" :keyboard="false" :maskClosable="false" @cancel="updateVisible(false)">
...
<!-- 
      ...
      <div class="mt-5 flex justify-end gap-5">
        <a-button type="grey" class="big shadow bold uppercase mb-5 mt-5" @click="updateVisible(false)">{{ t('取消') }}</a-button>

        <a-button v-if="activeKey === 2" type="dark" class="big shadow bold uppercase mb-5 mt-5" :loading="subLoading" @click="submitHandle">{{ t('提交') }}</a-button>
      </div>
    </a-modal>
  </div>
</template>
