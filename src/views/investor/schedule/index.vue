<template>
  <div>
    <div class="flex justify-end items-end" style="margin-top: -50px; margin-bottom: 20px; padding-right: 28px">
      <TableSearch v-if="isDetail || invest_id" @search="searchHandle"></TableSearch>
    </div>
    ...
    <a-spin :spinning="pageLoading" size="large">
      <div style="min-height: 200px">
        ...
        <div class="flex justify-between mb-3">
          <div>
            <vco-popconfirm
              v-if="isDetail && hasPermission('Investment:schedule:delete')"
              ...
              @update="getDataInfo()"
            >
              <a-button :disabled="!checkedIds.length">
                {{ t('删除') }}
              </a-button>
            </vco-popconfirm>
          </div>
        ...
        <div v-if="tabData.length" class="table-content">
          <div class="col-item th">
            <div class="item about flex items-center" @click="checkedAllHandle">
              <a-checkbox v-if="isDetail && hasPermission('Investment:schedule:delete')" :checked="Boolean(checkedAll)" :indeterminate="Boolean(indeterminate)"></a-checkbox>
            </div>
            ...
            <div class="item uppercase ops" v-if="isDetail && hasPermission('Investment:schedule:delete')">Ops</div>
          </div>

          <div class="col-content">
            <div v-for="(_item, _index) in tabData" :key="_item.date" class="col-item">
              <div class="item about flex">
                <a-checkbox v-if="isDetail && hasPermission('Investment:schedule:delete')" :checked="checkedIds.includes(_item.id)" @change="itemcheck(_item)"></a-checkbox>
              ...
              <div class="item ops cursor-pointer" v-if="isDetail && hasPermission('Investment:schedule:delete')">
