        ...
        <a-table ref="tableRef" rowKey="uuid" :columns="columns" :data-source="tableDataRef" table-layout="fixed" :pagination="false" :customRow="rowClick">
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'reason'">
              <span :title="record.data.reason" class="text-ellipsis overflow-hidden whitespace-normal line-clamp-3">{{ record.data.note }} </span>
            </template>
...
import { buyoutIndex } from '@/api/project/project';
