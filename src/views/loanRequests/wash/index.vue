        ...
        <a-table ref="tableRef" rowKey="uuid" :columns="columns" :data-source="tableDataRef" table-layout="fixed" :pagination="false" :customRow="rowClick">
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'name'">
              <span v-if="record.data.name">{{ record.data.name }}</span>
              <p v-else>--</p>
            </template>
...
import { projectWash } from '@/api/project/loanRequests';
...
const columns = reactive([
  ...
  { title: t('类型'), dataIndex: 'cate', width: 150, align: 'center', ellipsis: true },
  { title: t('状态t'), dataIndex: 'status_name', width: 150, align: 'center' },
...
const rowClick = (record, index) => {
  return {
    onClick: () => {
      navigationTo(`/projects/orgs?uuid=${record.project.uuid}&type=2`,true);
    }
  };
};
