<template>
  <a-modal :width="480" :open="open" :title="title" @cancel="closeModal" class="sys-form-content">
    <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
      <a-form-item name="mobile" :label="t('验证手机号')">
        <a-row :gutter="8">
          <a-col :span="6">
            <a-form-item-rest>
              <a-select v-model:value="form.pre" :options="preMobileOpts" disabled />
            </a-form-item-rest>
          </a-col>
          <a-col :span="14">
            <a-input v-model:value="form.mobile" :placeholder="t('验证手机号')" disabled />
          </a-col>
          <a-col :span="4" v-if="!verifyMobile.showCountdown">
            <a-form-item-rest>
              <a-button @click="handleVerify(false)" type="dark" block class="verify-btn big">
                {{ t('验证') }}
              </a-button>
            </a-form-item-rest>
          </a-col>
      ...
      <a-form-item name="code" :label="t('验证码V')" v-show="verifyMobile.showCode">
      ...
      <a-form-item name="newMobile" :label="t('新手机号')">
        <a-row :gutter="8">
          ...
          <a-col :span="14">
            <a-input v-model:value="form.newMobile" :placeholder="t('新手机号')" />
          </a-col>
          <a-col :span="4" v-if="!verifyMobile.showCountdownNew">
            <a-form-item-rest>
              <a-button @click="handleVerify(true)" type="dark" block class="verify-btn big">
                {{ t('验证') }}
              </a-button>
            </a-form-item-rest>
          </a-col>
      ...
      <a-form-item name="newCode" :label="t('验证码V')" v-show="verifyMobile.showCodeNew">
        <a-input v-model:value="form.newCode" :placeholder="t('验证码V')" />
      </a-form-item>
    </a-form>
    <template #footer>
      <div class="modal-footer">
        <a-button @click="save" size="large" type="primary" style="width: 40%" :loading="loading">
          {{ t('提交') }}
        </a-button>
      </div>
    </template>
