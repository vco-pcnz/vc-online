    ...
    <a-spin :spinning="tempLoading" size="large">
      <div class="main-info-container">
        <div v-if="!formLoading && !pageError">
          <process-header v-if="!queryError" :step-data="processStepData" :current="5" :status="infoData.status">
            <template #left>
...
<script setup>
  ...
  import { projectLmAuditDetail, projectDraftInfo } from "@/api/process"

  const { t } = useI18n();
  const { tempLoading, tempModule, currentId, queryError, simpleImage, processStepData } = useDynamicModule('five');
