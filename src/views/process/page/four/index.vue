    ...
    <a-spin :spinning="tempLoading" size="large">
      <div class="main-info-container">
        <div v-if="!formLoading && !pageError">
          <process-header v-if="!queryError" :step-data="processStepData" :current="4" :status="infoData.status">
            <template #left>
...
<script setup>
  ...
  import { projectApplyLoanInfo, projectDraftInfo } from "@/api/process"
  import { navigationTo } from "@/utils/tool";

  const { t } = useI18n();
  const { tempLoading, tempModule, currentId, queryError, simpleImage, processStepData } = useDynamicModule('four-view');
