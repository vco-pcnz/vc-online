      ...
      <div class="main-form-content">
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.dataIndex === 'typology'">
                <div class="stuff-item-content">
                  ...
                  <div class="stuff-item">
                    ...
                    >
                      <a-select-option v-for="num in 21" :key="`bath_${num - 1}`" :value="num - 1">{{num - 1}}</a-select-option>
                    </a-select>
                    <p>bath</p>
                  </div>
                  <div class="stuff-item">
              ...
              <template v-if="column.dataIndex === 'is_gst'">
                <a-switch v-model:checked="record.is_gst" />
              </template>
              ...
              <template v-if="column.dataIndex === 'opt'">
                <a-button type="link">{{ t('批量设置')}} </a-button>
              </template>
            </template>
          </a-table>
        </div>
        ...
        <div class="form-block-content">
          <div class="title flex">
            <p>{{ t('物品信息') }}</p>
            <a-button type="primary" size="small" shape="round">{{ t('添加') }}</a-button>
          </div>
          <a-table
            :columns="formColumns"
...
const getDataInfo = async () => {
  ...
  await projectDetailApi({
    ...
    if (res) {
      ...
      const {project_address_short, project_address, project_suburb, region_one_id, region_two_id, region_three_id, project_postcode} = res.project
...
const formColumns = reactive([
  ...
  { title: t('户型'), dataIndex: 'typology', width: 260, align: 'center' },
  { title: t('面积'), dataIndex: 'sqm', width: 120, align: 'center' },
  { title: t('销售价格'), dataIndex: 'sales_price', width: 140, align: 'center' },
  { title: t('含消费税'), dataIndex: 'is_gst', width: 115, align: 'center' },
  { title: t('担保价值'), dataIndex: 'amount', width: 140, align: 'center' },
