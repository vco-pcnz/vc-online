<template>
  <div>
    ...
    <div v-if="stepType === 2 && bonusItems.length" class="block-item mb" :class="{'checked': bonusInfo.is_check && showCheck}">
      ...
      <div class="sys-form-content mt-5">
        <a-form ref="bFormRef" layout="vertical" :model="bFormState" :rules="bFormRules">
          <a-row :gutter="24">
            <template v-if="bonusItems.length">
              <a-col v-for="item in bonusItems" :key="item.credit_table" class="data-col-item" :class="colClassName(bonusItems.length)">
                <a-form-item :label="item.credit_name" :name="item.credit_table">
                  <a-input
...
<script setup>
  ...
  const getFormItems = async () => {
    ...
    await ruleCredit({type: creditCate}).then(async (res) => {
      const data = res || []
      const writeData = data.filter(item => (item.is_write && item.type === 1))
      const perData = writeData.filter(item => item.is_ratio)
      const dolData = writeData.filter(item => !item.is_ratio)

      const bonusData = data.filter(item => (item.is_write && item.type === 2))

      if (props.stepType === 2) { // FC 审核，将奖金抽离
        const bRulesData = {}

        for (let i = 0; i < bonusData.length; i++) {
          bFormState.value[bonusData[i].credit_table] = bonusData[i].value

          bRulesData[bonusData[i].credit_table] = [{ validator: validateNum, trigger: 'blur' }]
          if (bonusData[i].is_req) {
            bRulesData[bonusData[i].credit_table].push({
              required: true, message: t('请输入') + bonusData[i].credit_name, trigger: 'blur'
            },
  ...
  const bonusCheckHandle = async () => {
    ...
    await fcAuditStatus(params).then(() => {
      emits('refresh')
      return true
    }).catch(() => {
      return false
