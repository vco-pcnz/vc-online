  ...
  <vco-page-panel @back="back">
    <template #title>
      <div class="page-title-content">
        ...
        <div class="tag uppercase">{{ t('买断') }}</div>
      </div>
    </template>
...
const panes = computed(() => {
  ...
  const variationsDetails = child.find((item) => item.path === `${mode}/projects/buyOut`);
...
const onChange = (key) => {
  router.push(`${mode}/projects/buyOut/${key}?uuid=${route.query.uuid}`);
};
