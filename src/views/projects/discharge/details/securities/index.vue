  ...
  <div>
    ...
    <vco-page-panel :title="$t('抵押物详情')" w="200px" @back="goBack">
      <div
        ...
        >{{ t('解押申请') }}</a-button>
        <a-button
          v-if="hasPermission('projects:securities:edit')"
          type="dark"
...
import { dischargeInfo } from '@/api/project/loan';
...
const showStatus = computed(() => {
  if (securityDetailInfo.value) {
    if (Number(securityDetailInfo.value.is_edit) === 1) {
      return {
        title: t('编辑中'),
        color: 'blue',
        key: 1
