  ...
  <div class="table-block text-center">
    ...
    <div v-if="tableData.length" class="table-body">
      <template v-for="(item, index) in tableData" :key="item.id">
        <ul class="table-col tr all-overdue" :class="{ active: active_id == item.id, declined: item.status_name === 'DECLINED DRAWDOWN', exceeded: Number(item?.over_money) > 0, isVsl: showLender }" @click="viewDetail(item)">
