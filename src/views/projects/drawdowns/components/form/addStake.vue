    ...
    <a-modal :width="900" :open="visible" :title="t('分配投资者')" :getContainer="() => $refs.JournalRef" :maskClosable="false" :footer="false" @cancel="updateVisible(false)">
      <div class="content">
        <div class="flex justify-end">
          <vco-choose-user v-if="visible" ref="vcoChooseUserRef" title="orgs search" url="stake/selStake" v-model:list="chooseUsers" :isMultiple="true" @change="checkUser">
        ...
        <div class="flex justify-center">
          <a-button @click="save" type="dark" class="save big uppercase" :disabled="distributableAmount < 0 || !orgs.length" :loading="loading">
            {{ t('保存') }}
          </a-button>
        </div>
      </div>
...
import { dsaveStake } from '@/api/project/loan';
...
const distributableAmount = computed(() => {
  ...
  console.log(sum);
  return props.detail?.amount - sum;
});
...
const save = () => {
  ...
  if (distributableAmount.value < 0) return;
  loading.value = true;
  let params = {
    uuid: props.uuid,
    id: props.detail.id,
