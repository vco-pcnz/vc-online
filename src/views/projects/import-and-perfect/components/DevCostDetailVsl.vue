            ...
            <template v-if="p_index === 0">
              <div class="flex justify-between tabel-type">
                <div class="flex gap-2">
                  ...
                  <a-button v-if="edit" type="brown" size="small" shape="round" @click="data.model = data.model ? 0 : 1">for {{ data.model ? t('QS报告') : t('进度放款') }}</a-button>
                  ...
                  <template v-if="column.dataIndex === 'change_value'">
                    <template v-if="record?.name !== 'Land GST'">
                      <div v-if="!record?.list || record?.name === 'Construction'" class="flex items-center gap-2">
                        <i class="iconfont" style="color: #eb4b6d">&#xe711;</i>
                        <a-input-number
                          v-if="edit"
                          v-model:value="record.change_value"
                          :max="Number(record.max_amount || 0)"
                          :min="0"
                          :formatter="(value) => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                          :parser="(value) => value.replace(/\$\s?|(,*)/g, '')"
                        />
                        <vco-number v-else :value="record.change_value" :precision="2" :color="'#eb4b6d'" size="fs_md" :bold="true" :end="true"></vco-number>
                        <a-tooltip v-if="edit">
                          <template #title>
                      ...
                      <template v-if="record?.list && record?.name !== 'Construction'">
                        <div v-for="(sub, subIndex) in record?.list" :key="subIndex" class="flex items-center gap-2 mt-2">
                          ...
                          <a-input-number
                            ...
                            :max="Number(sub.max_amount || 0)"
                            :min="0"
                            :formatter="(value) => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                            :parser="(value) => value.replace(/\$\s?|(,*)/g, '')"
                          />
                          <vco-number v-else :value="sub.change_value" :precision="2" :color="'#eb4b6d'" size="fs_md" :bold="true" :end="true"></vco-number>
                          <a-tooltip v-if="edit">
                            <template #title>
