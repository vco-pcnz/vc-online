<template>
  ...
  <div @click.stop ref="drawdownRequestRef" class="drawdown-request">
    <a-modal :width="700" :open="visible" :title="t('还款计算器')" :getContainer="() => $refs.drawdownRequestRef" :maskClosable="false" :footer="false" @cancel="updateVisible(false)">
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="overdueDays ? 8 : 12">
              <a-form-item :label="t('还款日期')" name="date">
                <a-date-picker v-model:value="formState.date" :disabledDate="disabledDateFormat" :format="selectDateFormat()" placeholder="" @change="dateChange">
                  <template #suffixIcon>
              ...
              <a-form-item class="w-full-label">
                <template #label>
                  <div class="flex justify-between items-center gap-3" style="height: 22px">
                    <p>Loan IRR</p>
                    <a-button v-if="maxReductionAmount && !isNormalUser" type="link" class="flex items-center text-sm" :loading="irrLoading" @click="refreshIrr">
                      {{ t('刷新') }}
                    </a-button>
                  </div>
                </template>
