  ...
  <a-modal :open="visible" :title="t('还款详情')" :width="830" :footer="null" :keyboard="false" :maskClosable="false" @cancel="updateVisible(false)">
    <a-row v-if="detailData && detailData?.id" :gutter="24">
      ...
      <template v-if="detailData.all_repayment">
        ...
        <a-col :span="6" class="item-txt">
          <p>
            Loan IRR <span v-if="detailData.reduction_irr_old" class="pl-2">{{ `(${numberStrFormat(detailData.reduction_irr_old)}%)` }}</span>
          </p>
    ...
    <div class="mt-10 mb-5">
      <div class="flex justify-end gap-5">
        <a-button type="brown" class="big shadow bold uppercase mb-5 mt-5" @click="updateVisible(false)">{{ t('关闭') }}</a-button>
...
import { repaymentDetail } from '@/api/project/loan';
...
const accept = async () => {
  ...
  await loanRsaveStep({ uuid: props.uuid, id: props.detailData?.id })
    .then((res) => {
      doneHandle();
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      repaymentDetail({ uuid: props.uuid, id: props.detailData?.id })
        .then((res) => {
