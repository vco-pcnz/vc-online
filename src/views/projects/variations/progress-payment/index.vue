<template>
  <div>
    <vco-page-panel @back="goBack" :title="t('进度付款')"></vco-page-panel>
    ...
    <a-spin :spinning="pageLoading" size="large">
      <div v-if="!pageError" class="progress-payment-content"> 
        ...
        <div class="form-block-content">
          ...
          >
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-else-if="column.dataIndex === 'payment'">
                <p>{{ Number(tool.times(Number(record[column.dataIndex]), 100)).toFixed(2) }}%</p>
              </template>
              <template v-else-if="column.dataIndex === 'total'">
                  ...
                  <a-table-summary-cell v-for="(item, index) in summaryCol" :index="index" :key="item.key" class="text-center">
                    ...
                    <template v-else-if="item.key === 'payment'">
                      <p class="total-percent"
                        :class="{'plus': summaryHandle(item.key) > 1, 'minus': summaryHandle(item.key) < 1}"
                      >{{ numberStrFormat(Number(tool.times(Number(summaryHandle(item.key)), 100)).toFixed(2)) }}%</p>
                    </template>
...
<script setup>
  ...
  const setTableHeader = () => {
    ...
    for (let i = 0; i < securityData.value.length; i++) {
      let title = securityData.value[i].card_no
      if (securityData.value[i].is_variation) {
        title = `${title}(${t('变更1')})`
      }
      headerData.push({
        title: title,
  ...
  const setAmortizedTable = () => {
    ...
    for (let i = 0; i < itemData.length; i++) {
      ...
      if (Boolean(itemData[i].status)) {
        title = `${title}(${t('已解押')}）`
      }
      if (itemData[i].is_variation) {
        title = `${title}(${t('变更1')})`
      }
      headerData.push({
        title,
  ...
  const getSetedData = async (footerData = []) => {
    ...
    try {
      let buildData = variationInfo.value.build || null
      if (!buildData || !buildData.data || !Object.keys(buildData.data).length) {
        buildData = await projectLoanGetBuild(params)
      }

      setedData.value = buildData
      // 首次放款数据
      if (Object.keys(buildData.payment).length) {
        if (buildData.payment[`0$1__payment`]) {
          advancePercent.value = buildData.payment[`0$1__payment`].amount
        }
      }
      ...
      if (Object.keys(buildData.summary).length) {
        if (buildData.summary[`${advanceKey.value}`]) {
          ...
          advanceObj.value = buildData.summary[`${advanceKey.value}`]
  ...
  onMounted(async () => {
    if (uuid.value && variationId.value) {
      await getProjectData()
    }
  })
</script>
