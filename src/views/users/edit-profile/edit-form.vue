  ...
  <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
      ...
      <a-row :gutter="8">
        <a-col :span="20">
          <a-form-item name="email" :label="t('邮箱')">
            <a-input v-model:value="form.email" :placeholder="t('邮箱')" :disabled="!!userInfo.email_ok" />
          </a-form-item>
        </a-col>
        <a-col :span="4">
          <a-form-item label=" ">
            <a-button @click="sendVerify" block class="validate_btn" :disabled="!!userInfo.email_ok">
              {{ t("变更") }}
            </a-button>
          </a-form-item>
        </a-col>
    ...
    <a-form-item :label="t('手机号')" name="mobile">
      <a-row :gutter="8">
        ...
        <a-col :span="16">
          <a-input v-model:value="form.mobile" :placeholder="t('手机号')" :disabled="!!userInfo.mobile_ok" />
        </a-col>
        <a-col :span="4">
          <a-form-item-rest>
            <a-button @click="sendVerify" block :disabled="!!userInfo.mobile_ok">
              {{ t("验证") }}
            </a-button>
          </a-form-item-rest>
        </a-col>
...
watch(
  () => props.userInfo,
  (val) => {
    const { firstName, middleName, lastName, email, pre, mobile } = val;
    assignFields({
      firstName,
      middleName,
      lastName,
